...
class VeiculosDAO:
  ...
  def todos(self):
    with sqlite3.connect('veiculos.db') as connection:
        ...
        veiculo_list = cursor.fetchall()
        
        veiculos: list[Veiculo] = []
        
        for v in veiculo_list:
            veiculo = Veiculo(id=v[0], nome=v[1], ano_fabricacao=v[2], ano_modelo=v[3], valor=v[4])
  ...
  def atualizar(self, id: int, veiculo: Veiculo):
    with sqlite3.connect('veiculos.db') as connect:
